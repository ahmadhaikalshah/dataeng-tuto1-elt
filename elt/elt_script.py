...
def wait_for_postgres(host, max_retries=5, delay_seconds=5):
    ...
    while retries < max_retries:
        ...
        try:
            result = run(['pg_isready', '-h', host], check=True, capture_output=True, text=True)
...
subprocess_env = dict(PGPASSWORD=source_config['password'])     # Set the PGPASSWORD environment variable to avoid password prompt
run(dump_command, env=subprocess_env, check=True)               # Execute the dump command
...
subprocess_env = dict(PGPASSWORD=destination_config['password'])    # Set the PGPASSWORD environment variable for the destination database
